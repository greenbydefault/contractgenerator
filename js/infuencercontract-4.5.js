document.addEventListener('DOMContentLoaded', function() {
  ...
  function updatePreview() {
    try {
      ...
      // NEU: Exklusivität in der Vorschau anzeigen (optional)
      const exclusivityValue = document.getElementById('exklusiv') ? document.getElementById('exklusiv').value : '';
      const previewExclusivityEl = document.getElementById('preview-exclusivity'); // Annahme: Es gibt ein Element mit dieser ID in der Vorschau
      if (previewExclusivityEl) {
          previewExclusivityEl.textContent = exclusivityValue || 'Keine Exklusivität ausgewählt';
          // Oder als HTML, falls komplexer:
          // previewExclusivityEl.innerHTML = exclusivityValue ? `<p>Exklusivität: ${exclusivityValue}</p>` : '<p>Keine Exklusivität ausgewählt</p>';
      }


      // NEU: Zusätzliche Informationen in der Vorschau anzeigen (optional)
      const extraInformationValue = document.getElementById('extra-information') ? document.getElementById('extra-information').value : '';
      const previewExtraInfoEl = document.getElementById('preview-extra-information'); // Annahme: Es gibt ein Element mit dieser ID in der Vorschau
      if (previewExtraInfoEl) {
          previewExtraInfoEl.textContent = extraInformationValue || 'Keine zusätzlichen Informationen';
          // Oder als HTML:
          // previewExtraInfoEl.innerHTML = extraInformationValue ? `<p>Zusätzliche Informationen: ${extraInformationValue.replace(/\n/g, '<br>')}</p>` : '<p>Keine zusätzlichen Informationen</p>';
      }
  ...
  function addTableOfContents(doc) {
    ...
    const paragraphs = [{
      ...
      num: "§2",
      title: "Plattformen & Veröffentlichung",
      page: 3
    }, {
      num: "§3",
      title: "Nutzung für Werbung (Media Buyout)",
      page: 4
    }, {
      num: "§4",
      ...
      num: "§7",
      title: "Qualität & Upload",
      page: 5
    }, {
      num: "§8",
      title: "Rechte Dritter & Musik",
      page: 5
    }, {
      num: "§9",
      title: "Werbekennzeichnung & Exklusivität",
      page: 6
    }, {
      num: "§10",
      title: "Verbindlichkeit Briefing & Skript",
      page: 6
    }, {
      num: "§11",
  ...
  function generatePDF() {
    ...
    try {
      ...
      const mediaBuyoutNo = mediaBuyoutNoEl ? mediaBuyoutNoEl.checked : false; // Unused, but good to have if logic changes
      ...
      const scriptDate = formatDate(scriptDateEl ? scriptDateEl.value : '') || '[Datum/Uhrzeit]'; // Keep original placeholder if needed
      ...
      const additionalCompNoEl = document.getElementById('additional-no'); // Unused, but good to have
      ...
      // NEU: Exklusivität
      const exclusivityEl = document.getElementById('exklusiv');
      const exclusivity = exclusivityEl ? exclusivityEl.value : ''; // Bleibt leer, falls nicht vorhanden/ausgewählt

      // NEU: Zusätzliche Informationen
      const extraInformationEl = document.getElementById('extra-information');
      const extraInformation = extraInformationEl ? extraInformationEl.value : ''; // Bleibt leer, falls nicht vorhanden/ausgefüllt
      ...
      y += 5;
      doc.text("eigenem Kanal", 30 + 7, y); // Assuming 7 is enough offset for the checkbox and space
      y += 6;
      ...
      y += 8; // Add some space before new page or next section


      // Neue Seite für §3 (falls nicht schon durch vorherige Inhalte ausgelöst)
      if (y > 250) { // Check if content is near the bottom of the page
          doc.addPage();
          y = 30;
      }
      ...
      if (mediaBuyoutYes) {
        ...
        renderCheckbox(doc, false, "Ja", 30, y); // Keep "Ja" unchecked
        y += 6; // Space between Ja and Nein
        renderCheckbox(doc, true, "Nein", 30, y); // "Nein" checked
        y += 8;
      ...
      xPos = 30 + doc.getTextWidth("Freigabe bis ");
      doc.setFont("helvetica", "bold");
      ...
      if (productionLocation && productionLocation !== '[Adresse]') {
        ...
        doc.text(".", xPos, y); // Add period at the end of production line if no location
      }

      y += 8; // Space after production details

      // Lieferung - mit Zeilenumbruch für bessere Lesbarkeit
      doc.text("Lieferung: Die Lieferung der finalen Inhalte erfolgt bis ", 30, y);
      xPos = 30 + doc.getTextWidth("Lieferung: Die Lieferung der finalen Inhalte erfolgt bis ");
      doc.setFont("helvetica", "bold");
      doc.text(deliveryDate, xPos, y);
      doc.setFont("helvetica", "normal");
      xPos += doc.getTextWidth(deliveryDate);
      doc.text(" / ", xPos, y);
      xPos += doc.getTextWidth(" / ");
      doc.setFont("helvetica", "bold");
      doc.text(deliveryTime, xPos, y);
      doc.setFont("helvetica", "normal");
      xPos += doc.getTextWidth(deliveryTime);
      doc.text(".", xPos, y);
      y += 8;

      // Veröffentlichung - mit Zeilenumbruch für bessere Lesbarkeit
      doc.text("Veröffentlichung: Die Veröffentlichung erfolgt am ", 30, y);
      xPos = 30 + doc.getTextWidth("Veröffentlichung: Die Veröffentlichung erfolgt am ");
      doc.setFont("helvetica", "bold");
      doc.text(publicationDate, xPos, y);
      doc.setFont("helvetica", "normal");
      xPos += doc.getTextWidth(publicationDate);
      doc.text(".", xPos, y);
      y += 12;


      // Neue Seite für §6 Vergütung (falls nicht schon durch vorherige Inhalte ausgelöst)
      if (y > 250) {
          doc.addPage();
          y = 30;
      }
      ...
      doc.text(compensation + " €", 30 + doc.getTextWidth("Die Nettovergütung beträgt") + 5, y);
      doc.setFont("helvetica", "normal");
      ...
      let initialYForPayment = y; // Store Y before checkbox rendering for alignment
      let checkboxX = 30 + doc.getTextWidth("Das Zahlungsziel beträgt") + 10; // X position for checkboxes

      renderCheckbox(doc, paymentTerm === "14 Tage", "14 Tage", checkboxX, initialYForPayment);
      initialYForPayment += 6;
      renderCheckbox(doc, paymentTerm === "30 Tage", "30 Tage", checkboxX, initialYForPayment);
      initialYForPayment += 6;
      renderCheckbox(doc, paymentTerm === "45 Tage", "45 Tage", checkboxX, initialYForPayment);
      y = initialYForPayment + 2; // Update y to be after the last checkbox + some space
      y += 8;


      // Zusätzliche Vergütung mit verbesserten Checkboxen
      doc.text("Eine zusätzliche Vergütung", 30, y);
      let initialYForAdditionalComp = y;
      let checkboxXAdditional = 30 + doc.getTextWidth("Eine zusätzliche Vergütung") + 10;

      renderCheckbox(doc, additionalCompNo, "ist nicht vereinbart.", checkboxXAdditional, initialYForAdditionalComp);
      initialYForAdditionalComp +=6;
      renderCheckbox(doc, additionalCompYes, "ist vereinbart: ", checkboxXAdditional, initialYForAdditionalComp);
      if (additionalCompYes) {
          doc.setFont("helvetica", "bold");
          doc.text(additionalCompText, checkboxXAdditional + doc.getTextWidth("ist vereinbart: ") + 7, initialYForAdditionalComp);
          doc.setFont("helvetica", "normal");
      }
      y = initialYForAdditionalComp + 2;
      y += 8;
      ...
      // Neue Seite für die restlichen Paragraphen (falls nicht schon durch vorherige Inhalte ausgelöst)
      if (y > 250) {
          doc.addPage();
          y = 30;
      }
      ...
      y += 8; // Space after advertising part

      // NEU: Exklusivität im PDF darstellen
      if (exclusivity) {
        doc.text("Exklusivität: ", 30, y);
        doc.setFont("helvetica", "bold");
        doc.text(exclusivity, 30 + doc.getTextWidth("Exklusivität: "), y);
        doc.setFont("helvetica", "normal");
        doc.text(".", 30 + doc.getTextWidth("Exklusivität: ") + doc.getTextWidth(exclusivity), y);
        y += 8;
      } else {
        doc.text("Es wurde keine spezifische Exklusivitätsvereinbarung getroffen.", 30, y);
        y += 8;
      }
      y += 4; // Extra space before next paragraph
      ...
      y += 8; // Space after confidentiality

      // NEU: Zusätzliche Informationen im PDF darstellen
      if (extraInformation) {
        y = addParagraphTitle(doc, "Zusätzliche Informationen", y); // Use a generic title or integrate into an existing paragraph
        // Split text by newlines and render each line to respect textarea formatting
        const lines = doc.splitTextToSize(extraInformation, 150); // Adjust maxWidth as needed
        doc.text(lines, 30, y);
        y += (lines.length * 5) + 4; // Adjust spacing based on number of lines
      }
      y += 4; // Extra space before next paragraph
  ...
  if (generateButton) {
    generateButton.addEventListener('click', function() {
      ...
      for (let i = 1; i <= 9; i++) { // Assuming 9 steps
        ...
        if (stepSection) {
          ...
          if (!stepIsValid) {
            allStepsValid = false;
            // Navigate to the first step with missing required fields and mark them
            goToStep(i);
            markInvalidFieldsInCurrentStep(); // Mark errors in the now current step
            showValidationError();
      ...
      if (allStepsValid) {
        ...
        // showValidationError() is called above if a step is invalid
      }
    });
  }
  ...
  document.querySelectorAll('[required], input, select, textarea').forEach(field => {
    field.addEventListener('input', function() {
      ...
      validateCurrentStep();
      updateProgress(); // Update progress based on real filled fields
    });
    // For select and checkbox/radio, 'change' event is more appropriate
  ...
  if (form && !document.querySelector('.container')) {
    ...
    container.className = 'container'; // Ensure this class is styled in your CSS
    parentElement.appendChild(container);

    // Sidebar erstellen
    const sidebar = document.createElement('div');
    sidebar.className = 'sidebar'; // Ensure this class is styled in your CSS
    ...
    mainContent.className = 'main-content'; // Ensure this class is styled in your CSS
  ...
  updateProgress();
  goToStep(1); // Ensure the first step is shown on load
});
